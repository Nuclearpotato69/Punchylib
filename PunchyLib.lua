	...
	function Window:AddTab(name)
		...
		local function makeGroupbox(gbName, side)
			...
			function GB:AddTextbox(k, c2)
				...
				local TX = {}
				TX.SetValue = function(_, v) inputBox.Text = tostring(v) end
				setmetatable(TX, {
					__index    = function(_,k2) if k2=="Value" then return inputBox.Text end end,
					__newindex = function(_,k2,v) if k2=="Value" then inputBox.Text=tostring(v) end end,
				})
	...
	function Window:AddESPPreview()
		...
		local function setupClone()
			...
			for _, p in ipairs(charClone:GetDescendants()) do
				if p:IsA("BasePart") then
					...
					if p~=cloneHRP then partRelCF[p]=cloneHRP.CFrame:ToObjectSpace(p.CFrame) end
				end
			end
			cloneHRP.CFrame = CFrame.new(0,0,0)
			for p, rel in pairs(partRelCF) do if p and p.Parent then p.CFrame=CFrame.new(0,0,0)*rel end end
			charClone.Parent = wm
		...
		task.spawn(function()
			if not setupClone() then
				for i=1,20 do task.wait(0.4); if setupClone() then break end end
			end
		end)
		...
		local nameTag = Create("TextLabel", {
			Size=UDim2.new(1,0,0,15), Position=UDim2.new(0,0,0,3),
			BackgroundTransparency=1, Text=LocalPlayer.DisplayName or "Player",
		...
		local distTag = Create("TextLabel", {
			Size=UDim2.new(1,0,0,11), Position=UDim2.new(0,0,0,19),
		...
		local BX,BW,BY,BH = 0.20,0.60,0.165,0.790
		local boxFrame = Create("Frame", {
			Size=UDim2.new(BW,0,BH,0), Position=UDim2.new(BX,0,BY,0),
			BackgroundTransparency=1, BorderSizePixel=0, ZIndex=8, Visible=false, Parent=vp,
		})
		...
		local fillFrame = Create("Frame", {
			...
			BackgroundTransparency=0.75, BorderSizePixel=0, ZIndex=7, Visible=false, Parent=boxFrame,
		})
		local hbBg = Create("Frame", {
			Size=UDim2.new(0,4,1,0), Position=UDim2.new(0,-8,0,0),
			BackgroundColor3=Color3.fromRGB(22,7,7), BorderSizePixel=0, ZIndex=8, Visible=false, Parent=boxFrame,
		...
		local skelFrame = Create("Frame", { Size=UDim2.new(1,0,1,0), BackgroundTransparency=1, BorderSizePixel=0, ZIndex=9, Visible=false, Parent=boxFrame })
		local skelLines = {}
		local function SkelLine(x1,y1,x2,y2)
			...
			local line = Create("Frame", {
				Size=UDim2.new(0,len,0,2), Position=UDim2.new(0,cx2-len/2,0,cy2-1),
				Rotation=angle, BackgroundColor3=espColor, BorderSizePixel=0, ZIndex=9, Parent=skelFrame,
			}, { Create("UICorner",{ CornerRadius=UDim.new(1,0) }) })
			table.insert(skelLines, line)
		end
		SkelLine(41,5,41,24); SkelLine(41,24,16,32); SkelLine(41,24,66,32)
		SkelLine(16,32,11,78); SkelLine(11,78,9,110); SkelLine(66,32,71,78); SkelLine(71,78,73,110)
		SkelLine(41,24,41,97); SkelLine(41,97,26,103); SkelLine(41,97,56,103)
		SkelLine(26,103,24,148); SkelLine(24,148,23,165); SkelLine(56,103,58,148); SkelLine(58,148,59,165)

		local chamHL = Instance.new("Highlight")
		chamHL.FillTransparency=0.45; chamHL.OutlineTransparency=1
		chamHL.FillColor=espColor; chamHL.DepthMode=Enum.HighlightDepthMode.AlwaysOnTop; chamHL.Enabled=false
		...
		local function updateColor(col)
			...
			fillFrame.BackgroundColor3=col; nameTag.TextColor3=col; chamHL.FillColor=col
		...
		local rotConn = RunService.Heartbeat:Connect(function(dt)
			...
			if cloneHRP and cloneHRP.Parent then
				...
				for p, rel in pairs(partRelCF) do if p and p.Parent then p.CFrame=newCF*rel end end
			end
			tickCount = tickCount + 1
			if tickCount >= 5 and bindMap then
				tickCount = 0
				local bm = bindMap
				local en      = bm.enabled     and Library.Toggles[bm.enabled]     and Library.Toggles[bm.enabled].Value
				local col     = bm.color        and Library.Options[bm.color]        and Library.Options[bm.color].Value
				local filled  = bm.filled       and Library.Toggles[bm.filled]       and Library.Toggles[bm.filled].Value
				local fillT   = bm.fillTrans    and Library.Options[bm.fillTrans]    and Library.Options[bm.fillTrans].Value
				local skel    = bm.skeleton     and Library.Toggles[bm.skeleton]     and Library.Toggles[bm.skeleton].Value
				local chamsEn = bm.chams        and Library.Toggles[bm.chams]        and Library.Toggles[bm.chams].Value
				local chamsC  = bm.chamsColor   and Library.Options[bm.chamsColor]   and Library.Options[bm.chamsColor].Value
				local chamsT  = bm.chamsTrans   and Library.Options[bm.chamsTrans]   and Library.Options[bm.chamsTrans].Value
				...
				if chamsEn ~= nil then
					...
					if chamsC then chamHL.FillColor = chamsC end
					if chamsT then chamHL.FillTransparency = chamsT/100 end
					if chamsEn and charClone and not chamHL.Adornee then chamHL.Adornee=charClone; chamHL.Parent=charClone end
				end
			end
		end)
