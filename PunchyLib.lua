...
-- ══════════════════════════════════════════════════════════════════════
--  KEYBIND OVERLAY  (completely redesigned)
-- ══════════════════════════════════════════════════════════════════════
local KBL_W = 224

local KeybindListFrame = Create("Frame", {
	Name                 = "KeybindList",
	Size                 = UDim2.new(0, KBL_W, 0, 0),
...
local kblHeader = Create("Frame", {
	...
	Create("UICorner", { CornerRadius = UDim.new(0, 8) }),
	-- fill bottom-half rounded corners
...
-- Count badge
local kblCountBadge = Create("TextLabel", {
...
local function refreshKBL()
	...
	for id, entry in pairs(Library._kblEntries) do
		...
		if b and b.key and entry and entry.frame and entry.frame.Parent then
			-- Update key badge text
			entry.keyLabel.Text = b.key.Name:upper()

			if b.mode == "toggle" then
				if b.state then
					-- Active toggle
					entry.stateChip.Text               = "ON"
					entry.stateChip.TextColor3          = T.TextHi
					entry.stateChip.BackgroundColor3    = T.AccentDim
					entry.bar.BackgroundColor3          = T.Accent
					entry.nameLabel.TextColor3          = T.TextHi
					local s = entry.stateChip:FindFirstChildOfClass("UIStroke")
					if s then s.Color = T.Accent end
				else
					-- Inactive toggle
					entry.stateChip.Text               = "OFF"
					entry.stateChip.TextColor3          = T.TextLo
					entry.stateChip.BackgroundColor3    = T.Bg3
					entry.bar.BackgroundColor3          = T.TextLo
					entry.nameLabel.TextColor3          = T.TextMid
					local s = entry.stateChip:FindFirstChildOfClass("UIStroke")
	...
	kblCountBadge.Text = tostring(count)
	kblCountBadge.TextColor3 = count > 0 and T.Accent or T.TextMid

	local shouldShow = count > 0
	if shouldShow ~= KeybindListFrame.Visible then
		if shouldShow then
			KeybindListFrame.Visible = true
			KeybindListFrame.Position = UDim2.new(0, 12, 1, -6)
			Tween(KeybindListFrame, { Position = UDim2.new(0, 12, 1, -12) }, 0.18)
		else
			Tween(KeybindListFrame, { Position = UDim2.new(0, 12, 1, -6) }, 0.14)
			task.delay(0.15, function()
				if not (count > 0) then
					KeybindListFrame.Visible = false
					KeybindListFrame.Position = UDim2.new(0, 12, 1, -12)
				end
			end)
		end
	end
...
function Library:SaveConfig(name, folder)
	...
	if ok then Library:Notify("Saved: "..name, 2, "success")
	else Library:Notify("Save failed!", 2, "error") end
end
...
function Library:DeleteConfig(name, folder)
	...
	if ok then Library:Notify("Deleted: "..name, 2, "success")
	else Library:Notify("Delete failed!", 2, "error") end
end
...
local function MakeColorPicker(k, defaultColor, cb)
	...
	local satOvl = Create("Frame", { Size = UDim2.new(1,0,1,0), BackgroundColor3 = Color3.new(1,1,1), BorderSizePixel = 0, ZIndex = 602, Parent = svArea })
	Create("UIGradient", { Transparency = NumberSequence.new({ NumberSequenceKeypoint.new(0,0), NumberSequenceKeypoint.new(1,1) }), Rotation = 0, Parent = satOvl })
	local valOvl = Create("Frame", { Size = UDim2.new(1,0,1,0), BackgroundColor3 = Color3.new(0,0,0), BorderSizePixel = 0, ZIndex = 603, Parent = svArea })
	Create("UIGradient", { Transparency = NumberSequence.new({ NumberSequenceKeypoint.new(0,1), NumberSequenceKeypoint.new(1,0) }), Rotation = 90, Parent = valOvl })

	local svCursor = Create("Frame", {
		Size = UDim2.new(0,10,0,10), AnchorPoint = Vector2.new(0.5,0.5), Position = UDim2.new(s,0,1-v,0),
		BackgroundColor3 = Color3.new(1,1,1), BorderSizePixel = 0, ZIndex = 604, Parent = svArea,
	}, { Create("UICorner",{ CornerRadius = UDim.new(1,0) }), Create("UIStroke",{ Color = Color3.new(0,0,0), Thickness = 1.5 }) })

	local hueBar = Create("Frame", {
		Size = UDim2.new(0,HUE_W,0,SV_H), Position = UDim2.new(0,PAD+SV_W+PAD/2,0,PAD),
		BorderSizePixel = 0, ClipsDescendants = true, ZIndex = 601, Parent = pickerFrame,
	}, { Create("UICorner",{ CornerRadius = UDim.new(0,3) }) })
	...
	local hueCursor = Create("Frame", {
		Size = UDim2.new(1,4,0,3), AnchorPoint = Vector2.new(0.5,0.5), Position = UDim2.new(0.5,0,h,0),
		BackgroundColor3 = Color3.new(1,1,1), BorderSizePixel = 0, ZIndex = 602, Parent = hueBar,
	}, { Create("UICorner",{ CornerRadius = UDim.new(1,0) }), Create("UIStroke",{ Color = Color3.new(0,0,0), Thickness = 1 }) })

	local bottomY = PAD + SV_H + 8
	local previewSwatch = Create("Frame", {
		Size = UDim2.new(0,22,0,18), Position = UDim2.new(0,PAD,0,bottomY+2),
		BackgroundColor3 = currentColor, BorderSizePixel = 0, ZIndex = 601, Parent = pickerFrame,
	}, { Create("UICorner",{ CornerRadius = UDim.new(0,3) }), Create("UIStroke",{ Color = T.BorderHi, Thickness = 1 }) })
	local hexBox = Create("TextBox", {
		Size = UDim2.new(0,totalW-PAD*2-28,0,22), Position = UDim2.new(0,PAD+28,0,bottomY),
		BackgroundColor3 = T.Input, BorderSizePixel = 0, Text = Color3toHex(currentColor),
		TextColor3 = T.TextHi, PlaceholderText = "#RRGGBB", PlaceholderColor3 = T.TextLo,
		TextSize = 10, Font = Enum.Font.GothamMedium, TextXAlignment = Enum.TextXAlignment.Left,
		ClearTextOnFocus = false, ZIndex = 601, Parent = pickerFrame,
	}, { Create("UICorner",{ CornerRadius = UDim.new(0,3) }), Create("UIStroke",{ Color = T.BorderHi, Thickness = 1 }), Create("UIPadding",{ PaddingLeft = UDim.new(0,6) }) })
	...
	local function updateFromHSV(nh, ns, nv)
		...
		local opt = Library.Options[k]; if opt then opt._val = currentColor end
	end

	local svDrag, hueDrag = false, false
	local function svFromMouse(mx,my) local abs=svArea.AbsolutePosition; local siz=svArea.AbsoluteSize; updateFromHSV(h,math.clamp((mx-abs.X)/siz.X,0,1),1-math.clamp((my-abs.Y)/siz.Y,0,1)) end
	local function hueFromMouse(my)  local abs=hueBar.AbsolutePosition;  local siz=hueBar.AbsoluteSize;  updateFromHSV(math.clamp((my-abs.Y)/siz.Y,0,1),s,v) end
	svArea.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then svDrag=true; svFromMouse(i.Position.X,i.Position.Y) end end)
	hueBar.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then hueDrag=true; hueFromMouse(i.Position.Y) end end)
	local c1=UserInputService.InputChanged:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseMovement then if svDrag then svFromMouse(i.Position.X,i.Position.Y) end; if hueDrag then hueFromMouse(i.Position.Y) end end end)
	local c2=UserInputService.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then svDrag=false; hueDrag=false end end)
	table.insert(Library._connections,c1); table.insert(Library._connections,c2)
	...
	hexBox.FocusLost:Connect(function()
		...
		local nc=HextoColor3(hexBox.Text); if nc then local nh,ns,nv=Color3.toHSV(nc); updateFromHSV(nh,ns,nv) else hexBox.Text=Color3toHex(currentColor) end
	end)
	local cpConn = ColorPickerOverlay.InputBegan:Connect(function(inp)
		if inp.UserInputType==Enum.UserInputType.MouseButton1 then
			local mp=UserInputService:GetMouseLocation(); local pos=pickerFrame.AbsolutePosition; local siz=pickerFrame.AbsoluteSize
			if mp.X<pos.X or mp.X>pos.X+siz.X or mp.Y<pos.Y or mp.Y>pos.Y+siz.Y then pickerOpen=false; pickerFrame.Visible=false; ColorPickerOverlay.Visible=false end
		end
	end)
	table.insert(Library._connections, cpConn)

	local function openPicker(swatch)
		local abs=swatch.AbsolutePosition; local camSz=workspace.CurrentCamera and workspace.CurrentCamera.ViewportSize or Vector2.new(1920,1080)
		local px=abs.X+swatch.AbsoluteSize.X+4; local py=abs.Y-4
		if px+totalW>camSz.X then px=abs.X-totalW-4 end
		if py+totalH>camSz.Y then py=camSz.Y-totalH-4 end
		if py<0 then py=4 end
		pickerFrame.Position=UDim2.new(0,px,0,py); pickerOpen=true; pickerFrame.Visible=true; ColorPickerOverlay.Visible=true; updateFromHSV(h,s,v)
	end
	return pickerFrame, openPicker, function() return currentColor end, function(nc)
		local nh,ns,nv=Color3.toHSV(nc); h,s,v=nh,ns,nv; currentColor=nc
		svArea.BackgroundColor3=Color3.fromHSV(h,1,1); svCursor.Position=UDim2.new(s,0,1-v,0); hueCursor.Position=UDim2.new(0.5,0,h,0); previewSwatch.BackgroundColor3=currentColor; hexBox.Text=Color3toHex(currentColor)
	end
end
...
function Library:CreateWindow(cfg)
	...
	Create("Frame", { Size=UDim2.new(0,80,0,2), Position=UDim2.new(0,20,0,0), BackgroundColor3=T.Accent, BorderSizePixel=0, ZIndex=2, Parent=WinFrame }, { Create("UICorner",{ CornerRadius=UDim.new(1,0) }) })

	local Titlebar = Create("Frame", {
		Size=UDim2.new(1,0,0,TITLE_H), BackgroundColor3=T.Bg0, BorderSizePixel=0, Parent=WinFrame,
	}, {
		Create("UICorner",{ CornerRadius=UDim.new(0,6) }),
	...
	Create("TextLabel", { Size=UDim2.new(0,80,1,0), Position=UDim2.new(0,30,0,0), BackgroundTransparency=1, Text=title:upper(), TextColor3=T.TextHi, TextSize=12, Font=Enum.Font.GothamBlack, TextXAlignment=Enum.TextXAlignment.Left, Parent=Titlebar })
	if subtitle ~= "" then
		...
		Create("TextLabel",{ Size=UDim2.new(0,140,1,0), Position=UDim2.new(0,120,0,0), BackgroundTransparency=1, Text=subtitle, TextColor3=T.TextMid, TextSize=10, Font=Enum.Font.Gotham, TextXAlignment=Enum.TextXAlignment.Left, Parent=Titlebar })
	end

	local keyBadge = Create("TextLabel", {
		Size=UDim2.new(0,58,0,17), Position=UDim2.new(1,-88,0.5,-8), BackgroundColor3=T.Bg2, BorderSizePixel=0,
		Text=keyRef.v.Name:upper(), TextColor3=T.TextMid, TextSize=9, Font=Enum.Font.GothamMedium, Parent=Titlebar,
	}, { Create("UIStroke",{ Color=T.BorderHi, Thickness=1 }), Create("UICorner",{ CornerRadius=UDim.new(0,3) }) })

	local CloseBtn = Create("TextButton", {
		Size=UDim2.new(0,12,0,12), Position=UDim2.new(1,-20,0.5,-6), BackgroundColor3=T.Red, BorderSizePixel=0,
	...
	local function openWindow()
		if isAnimating then return end; isAnimating = true
		local p = WinFrame.Position
		WinFrame.Position=UDim2.new(p.X.Scale,p.X.Offset,p.Y.Scale,p.Y.Offset-8); WinFrame.Visible=true
	...
	local TabBar = Create("Frame", {
		Size=UDim2.new(1,0,0,TAB_H), Position=UDim2.new(0,0,0,TITLE_H), BackgroundColor3=T.Bg0, BorderSizePixel=0, Parent=WinFrame,
	}, { Create("UIListLayout",{ FillDirection=Enum.FillDirection.Horizontal, SortOrder=Enum.SortOrder.LayoutOrder }), Create("UIPadding",{ PaddingLeft=UDim.new(0,6) }) })
	...
	local ContentArea = Create("ScrollingFrame", {
		Name="ContentArea", Size=UDim2.new(1,0,0,CONTENT_H), Position=UDim2.new(0,0,0,TITLE_H+TAB_H),
		...
		AutomaticCanvasSize=Enum.AutomaticSize.Y, ClipsDescendants=true, Parent=WinFrame,
	})

	local StatusBar = Create("Frame", {
		Size=UDim2.new(1,0,0,STATUS_H), Position=UDim2.new(0,0,0,WIN_H-STATUS_H), BackgroundColor3=T.Bg0, BorderSizePixel=0, Parent=WinFrame,
	}, { Create("UICorner",{ CornerRadius=UDim.new(0,6) }), Create("Frame",{ Size=UDim2.new(1,0,0,8), BackgroundColor3=T.Bg0, BorderSizePixel=0 }), Create("Frame",{ Size=UDim2.new(1,0,0,1), BackgroundColor3=T.Border, BorderSizePixel=0 }) })
	...
	local PingLabel = Create("TextLabel",{ Size=UDim2.new(0,80,1,0), Position=UDim2.new(1,-85,0,0), BackgroundTransparency=1, Text="12ms · v3.1", TextColor3=T.TextLo, TextSize=9, Font=Enum.Font.GothamMedium, TextXAlignment=Enum.TextXAlignment.Right, Parent=StatusBar })
	task.spawn(function()
		while task.wait(2.5) do
			if not WinFrame or not WinFrame.Parent then break end
			local ms=math.random(6,26); PingLabel.Text=ms.."ms · v3.1"; PingLabel.TextColor3=ms>20 and Color3.fromRGB(210,160,50) or T.TextLo
		end
	end)

	local menuKeyConn = UserInputService.InputBegan:Connect(function(input,gpe)
		if not gpe and input.KeyCode==keyRef.v then if WinFrame.Visible then closeWindow() else openWindow() end end
	end)
	table.insert(Library._connections, menuKeyConn)

	local Window = { _frame=WinFrame, _tabBar=TabBar, _content=ContentArea, _tabs={}, _activeTab=nil, _keyRef=keyRef, _keyBadge=keyBadge }

	function Window:_switchTab(tab)
		for _, t in ipairs(self._tabs) do
			t._page.Visible=false; Tween(t._btn,{ TextColor3=T.TextMid },0.12); t._indicator.BackgroundTransparency=1
		end
		tab._page.Visible=true; Tween(tab._btn,{ TextColor3=T.Accent },0.12); tab._indicator.BackgroundTransparency=0; self._activeTab=tab
	end
	...
	function Window:AddTab(name)
		...
		local indicator = Create("Frame", { Size=UDim2.new(1,-16,0,2), Position=UDim2.new(0,8,1,-2), BackgroundColor3=T.Accent, BackgroundTransparency=1, BorderSizePixel=0, Parent=btn }, { Create("UICorner",{ CornerRadius=UDim.new(1,0) }) })
		...
		local function getOrMakeCol(side)
			...
			if not tab[which] then
				tab[which] = Create("Frame", { Size=UDim2.new(0.5,-4,0,0), AutomaticSize=Enum.AutomaticSize.Y, BackgroundTransparency=1, LayoutOrder=side=="Left" and 1 or 2, Parent=page }, { Create("UIListLayout",{ SortOrder=Enum.SortOrder.LayoutOrder, Padding=UDim.new(0,8) }) })
			end
			return tab[which]
		end
		...
		local function makeGroupbox(gbName, side)
			...
			local box = Create("Frame", { Size=UDim2.new(1,0,0,0), AutomaticSize=Enum.AutomaticSize.Y, BackgroundColor3=T.Bg2, BorderSizePixel=0, LayoutOrder=#col:GetChildren(), Parent=col }, { Create("UIStroke",{ Color=T.Border, Thickness=1 }), Create("UICorner",{ CornerRadius=UDim.new(0,5) }) })
			...
			function GB:AddToggle(k, c2)
				...
				local accentCol=color=="purple" and T.Purple or T.Accent; local accentDim=color=="purple" and T.PurpleDim or T.AccentDim
				...
				local function apply(v2,silent)
					value=v2
					if v2 then Tween(pill,{ BackgroundColor3=accentDim },0.15); Tween(thumb,{ Position=UDim2.new(0,18,0.5,-5), BackgroundColor3=accentCol },0.15); stroke.Color=accentCol
					else Tween(pill,{ BackgroundColor3=T.ToggleOff },0.15); Tween(thumb,{ Position=UDim2.new(0,2,0.5,-5), BackgroundColor3=T.TextLo },0.15); stroke.Color=T.BorderHi end
				...
				local To={}; To.SetValue=function(_,v2) apply(v2) end
			...
			function GB:AddSlider(k, c2)
				...
				local valLabel=Create("TextLabel",{ Size=UDim2.new(0.4,0,0,18), Position=UDim2.new(0.6,0,0,2), BackgroundTransparency=1, Text=tostring(default)..suffix, TextColor3=ac, TextSize=11, Font=Enum.Font.GothamBold, TextXAlignment=Enum.TextXAlignment.Right, Parent=wrap })
				...
				local function applyPct(pct)
					...
					if rounding==0 then value=math.round(raw) else local m=10^rounding; value=math.floor(raw*m+0.5)/m end
					fill.Size=UDim2.new(pct,0,1,0); thumb.Position=UDim2.new(pct,-7,0.5,-7); valLabel.Text=tostring(value)..suffix; pcall(cb,value)
				end
				applyPct((default-min)/(max-min))
				thumb.MouseButton1Down:Connect(function() slDrag=true; Tween(thumb,{ Size=UDim2.new(0,16,0,16), Position=UDim2.new(thumb.Position.X.Scale,thumb.Position.X.Offset-1,0.5,-8) },0.08) end)
				track.InputBegan:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.MouseButton1 then slDrag=true; applyPct((UserInputService:GetMouseLocation().X-track.AbsolutePosition.X)/track.AbsoluteSize.X) end end)
				UserInputService.InputChanged:Connect(function(inp) if slDrag and inp.UserInputType==Enum.UserInputType.MouseMovement then applyPct((inp.Position.X-track.AbsolutePosition.X)/track.AbsoluteSize.X) end end)
				UserInputService.InputEnded:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.MouseButton1 and slDrag then slDrag=false; Tween(thumb,{ Size=UDim2.new(0,14,0,14), Position=UDim2.new(thumb.Position.X.Scale,thumb.Position.X.Offset+1,0.5,-7) },0.08) end end)
				local S={}; S.SetValue=function(_,v2) applyPct((v2-min)/(max-min)) end
				setmetatable(S,{ __index=function(_,k2) if k2=="Value" then return value end end, __newindex=function(_,k2,v2) if k2=="Value" then applyPct((v2-min)/(max-min)) end end })
				Library.Options[k]=S; return S
			end

			function GB:AddDropdown(k, c2)
				c2=c2 or {}
				local text=c2.Text or k; local vals=c2.Values or {}; local default=c2.Default or (vals[1] or "")
				...
				local dropBtn=Create("TextButton",{ Size=UDim2.new(1,0,0,24), Position=UDim2.new(0,0,0,22), BackgroundColor3=T.Input, BorderSizePixel=0, Text="", Parent=wrap },{ Create("UIStroke",{ Color=T.BorderHi, Thickness=1 }), Create("UICorner",{ CornerRadius=UDim.new(0,3) }) })
				local selLabel=Create("TextLabel",{ Size=UDim2.new(1,-24,1,0), Position=UDim2.new(0,8,0,0), BackgroundTransparency=1, Text=multi and "None" or default, TextColor3=T.TextHi, TextSize=11, Font=Enum.Font.Gotham, TextXAlignment=Enum.TextXAlignment.Left, Parent=dropBtn })
				...
				local listFrame=Create("Frame",{ Size=UDim2.new(1,0,0,0), Position=UDim2.new(0,0,1,3), BackgroundColor3=T.Bg3, BorderSizePixel=0, Visible=false, ZIndex=25, Parent=dropBtn },{ Create("UIStroke",{ Color=T.BorderHi, Thickness=1 }), Create("UICorner",{ CornerRadius=UDim.new(0,3) }), Create("UIListLayout",{ SortOrder=Enum.SortOrder.LayoutOrder }) })
				local value=multi and {} or default; local open=false
				local function getDisplayText()
					if not multi then return value end
					local keys={}; for k2,_ in pairs(value) do table.insert(keys,k2) end
					if #keys==0 then return "None" elseif #keys==1 then return keys[1] else return #keys.." selected" end
				end
				local function buildList()
					...
					for _,v2 in ipairs(vals) do
						...
						local opt=Create("TextButton",{ Size=UDim2.new(1,0,0,24), BackgroundColor3=T.Bg3, BorderSizePixel=0, Text=v2, TextColor3=isSelected and T.Accent or T.TextHi, TextSize=11, Font=isSelected and Enum.Font.GothamMedium or Enum.Font.Gotham, ZIndex=26, Parent=listFrame },{ Create("UIPadding",{ PaddingLeft=UDim.new(0,10) }) })
						opt.TextXAlignment=Enum.TextXAlignment.Left
						if multi then
							Create("TextLabel",{ Size=UDim2.new(0,18,1,0), Position=UDim2.new(1,-20,0,0), BackgroundTransparency=1, Text=isSelected and "✓" or "", TextColor3=T.Accent, TextSize=11, Font=Enum.Font.GothamBold, ZIndex=27, Parent=opt })
							opt.MouseButton1Click:Connect(function() if value[v2] then value[v2]=nil else value[v2]=true end; selLabel.Text=getDisplayText(); pcall(cb,value); buildList() end)
						else
							opt.MouseButton1Click:Connect(function() value=v2; selLabel.Text=v2; open=false; listFrame.Visible=false; pcall(cb,v2); buildList() end)
						end
				...
				buildList()
				dropBtn.MouseButton1Click:Connect(function() open=not open; listFrame.Visible=open; Tween(arrowL,{ Rotation=open and -35 or 35 },0.1); Tween(arrowR,{ Rotation=open and 35 or -35 },0.1) end)
				...
				local D={Values=vals}
				D.SetValue=function(_,v2) if multi then if type(v2)=="table" then value=v2 else value[v2]=true end else value=v2 end; selLabel.Text=getDisplayText(); pcall(cb,value); buildList() end
			...
			function GB:AddColorPicker(k, c2)
				c2=c2 or {}
				local text=c2.Text or k; local default=c2.Default or T.Accent; local cb=c2.Callback or function() end
				...
				local swatch=Create("TextButton",{ Size=UDim2.new(0,20,0,14), Position=UDim2.new(1,-20,0.5,-7), BackgroundColor3=default, BorderSizePixel=0, Text="", Parent=row },{ Create("UIStroke",{ Color=T.BorderHi, Thickness=1 }), Create("UICorner",{ CornerRadius=UDim.new(0,3) }) })
				local currentColor=default
				local _,openPicker,getColor,setPickerColor=MakeColorPicker(k,default,function(nc) currentColor=nc; swatch.BackgroundColor3=nc; pcall(cb,nc) end)
				swatch.MouseButton1Click:Connect(function() openPicker(swatch) end)
				local CP={_val=default}
				CP.SetValue=function(_,v2) currentColor=v2; swatch.BackgroundColor3=v2; setPickerColor(v2); pcall(cb,v2) end
				setmetatable(CP,{ __index=function(_,k2) if k2=="Value" then return currentColor end end, __newindex=function(_,k2,v2) if k2=="Value" then CP:SetValue(v2) end end })
			...
			function GB:AddKeybind(k, c2)
				...
				-- ── Keybind overlay entry (redesigned) ───────────────────
				...
				-- Left colored state bar
				local kblBar = Create("Frame", {
					Size             = UDim2.new(0, 3, 0, 16),
					Position         = UDim2.new(0, 10, 0.5, -8),
					BackgroundColor3 = T.TextLo,
					BorderSizePixel  = 0,
					Parent           = kblEntry,
				}, { Create("UICorner",{ CornerRadius = UDim.new(1,0) }) })
				...
				local kblState = Create("TextLabel", {
					...
					BackgroundColor3   = T.Bg3,
					BorderSizePixel    = 0,
					Text               = mode == "hold" and "HOLD" or "OFF",
					TextColor3         = T.TextLo,
					TextSize           = 8,
					Font               = Enum.Font.GothamBold,
					TextXAlignment     = Enum.TextXAlignment.Center,
				...
				-- Hover events on the entry row
				kblEntry.MouseEnter:Connect(function() Tween(kblHover,{ BackgroundTransparency=0.6 },0.1) end)
				kblEntry.MouseLeave:Connect(function() Tween(kblHover,{ BackgroundTransparency=1 },0.1) end)

				-- FIX: store as plain Lua table
				...
				row.MouseButton1Click:Connect(function()
					binding=true; badge.Text="[ . . . ]"; badge.TextColor3=T.Accent; Tween(badge,{ BackgroundColor3=T.Bg3 },0.1)
				end)
				row.MouseEnter:Connect(function() row.BackgroundColor3=T.Hover; row.BackgroundTransparency=0 end)
				row.MouseLeave:Connect(function() row.BackgroundTransparency=1 end)
				UserInputService.InputBegan:Connect(function(inp,gpe)
					if binding and not gpe and inp.UserInputType==Enum.UserInputType.Keyboard then
						if inp.KeyCode==Enum.KeyCode.Escape then binding=false; badge.Text="["..value.Name:upper().."]"; badge.TextColor3=T.TextMid; Tween(badge,{ BackgroundColor3=T.Input },0.1); return end
						...
						Library:RegisterBind(k,value,mode,cb,holdCb); pcall(cb,value)
					end
				end)
			...
			function GB:AddTextbox(k, c2)
				c2=c2 or {}
				local text=c2.Text or k; local default=c2.Default or ""; local placeholder=c2.Placeholder or "Enter value..."
				...
				inputBox.FocusLost:Connect(function(enter) Tween(stroke,{ Color=T.BorderHi },0.12); pcall(cb,inputBox.Text,enter) end)
				local TX={}; TX.SetValue=function(_,v2) inputBox.Text=tostring(v2) end
				setmetatable(TX,{ __index=function(_,k2) if k2=="Value" then return inputBox.Text end end, __newindex=function(_,k2,v2) if k2=="Value" then inputBox.Text=tostring(v2) end end })
			...
			function GB:AddButton(c2)
				...
				btn.MouseButton1Click:Connect(function() Tween(btn,{ BackgroundColor3=T.AccentDim },0.08); task.wait(0.14); Tween(btn,{ BackgroundColor3=T.Bg3 },0.14); pcall(cb) end)
			...
			function GB:AddLabel(text, id)
				...
				if id then
					local L={}; L.SetText=function(_,t) lbl.Text=t end; L.SetColor=function(_,c3) lbl.TextColor3=c3 end; Library.Options[id]=L; return L
				end
			end
			...
			function GB:BuildConfigList(folder)
				...
				-- FIX: forward-declare so Save callback closure can reference it
				local profileDropdown
				self:AddButton({ Text="Save Config", Callback=function()
					Library:SaveConfig(nameBox.Value,folder)
					if profileDropdown then profileDropdown:SetValue(nameBox.Value) end
				end })
				...
				profileDropdown=self:AddDropdown("__cfg_profile",{ Text="Load Profile", Values=getProfiles(), Default=profileNames[1] or "" })
				self:AddButton({ Text="Load Config", Callback=function()
					local sel=profileDropdown.Value; if sel and sel~="No configs found" then Library:LoadConfig(sel,folder) end
				end })
				self:AddButton({ Text="Delete Config", Callback=function()
					local sel=profileDropdown.Value
					if sel and sel~="No configs found" then Library:DeleteConfig(sel,folder); local u=getProfiles(); profileDropdown.Values=u; profileDropdown:SetValue(u[1] or "") end
				end })
				self:AddButton({ Text="Refresh List", Callback=function()
					local u=getProfiles(); profileDropdown.Values=u; profileDropdown:SetValue(u[1] or "")
				end })
			end
	...
	function Window:AddESPPreview()
		...
		local PW,PH=148,270
		local previewFrame=Create("Frame",{ Name="ESPPreview", Size=UDim2.new(0,PW,0,PH), Position=UDim2.new(1,14,0,0), BackgroundColor3=T.Bg1, BorderSizePixel=0, Visible=true, Parent=WinFrame },{ Create("UIStroke",{ Color=T.BorderHi, Thickness=1 }), Create("UICorner",{ CornerRadius=UDim.new(0,6) }) })
		Create("Frame",{ Size=UDim2.new(0,56,0,2), Position=UDim2.new(0,10,0,0), BackgroundColor3=T.Accent, BorderSizePixel=0, ZIndex=2, Parent=previewFrame },{ Create("UICorner",{ CornerRadius=UDim.new(1,0) }) })
		local hdr=Create("Frame",{ Size=UDim2.new(1,0,0,28), BackgroundColor3=T.Bg0, BorderSizePixel=0, Parent=previewFrame },{ Create("UICorner",{ CornerRadius=UDim.new(0,6) }), Create("Frame",{ Size=UDim2.new(1,0,0,8), Position=UDim2.new(0,0,1,-8), BackgroundColor3=T.Bg0, BorderSizePixel=0 }), Create("Frame",{ Size=UDim2.new(1,0,0,1), Position=UDim2.new(0,0,1,-1), BackgroundColor3=T.Border, BorderSizePixel=0 }) })
		...
		local accentDot=Create("Frame",{ Size=UDim2.new(0,6,0,6), Position=UDim2.new(1,-12,0.5,-3), BackgroundColor3=T.Accent, BorderSizePixel=0, Parent=hdr },{ Create("UICorner",{ CornerRadius=UDim.new(1,0) }) })
		MakeDraggable(previewFrame,hdr)
		local VP_H=PH-28
		local vp=Create("ViewportFrame",{ Size=UDim2.new(1,0,0,VP_H), Position=UDim2.new(0,0,0,28), BackgroundColor3=Color3.fromRGB(7,6,13), Ambient=Color3.fromRGB(110,105,140), LightDirection=Vector3.new(-0.6,-1,-0.5), LightColor=Color3.fromRGB(255,248,235), ClipsDescendants=true, Parent=previewFrame },{ Create("UICorner",{ CornerRadius=UDim.new(0,6) }) })
		...
		local function setupClone()
			...
			for _,d in ipairs(charClone:GetDescendants()) do if d:IsA("Script") or d:IsA("LocalScript") or d:IsA("ModuleScript") or d:IsA("Animator") or d:IsA("Animation") or d:IsA("BodyMover") or d:IsA("BillboardGui") or d:IsA("SurfaceGui") or d:IsA("Highlight") or d:IsA("SelectionBox") or d:IsA("ParticleEmitter") then pcall(function() d:Destroy() end) end end
		...
		local weaponTag=Create("TextLabel",{ Size=UDim2.new(1,0,0,10), Position=UDim2.new(0,0,0,28), BackgroundTransparency=1, Text="AK-47", TextColor3=T.TextMid, TextSize=7, Font=Enum.Font.Gotham, TextXAlignment=Enum.TextXAlignment.Center, TextStrokeTransparency=0.5, ZIndex=12, Visible=false, Parent=vp })
		...
		local armorFill=Create("Frame",{ Size=UDim2.new(1,0,0.55,0), Position=UDim2.new(0,0,0.45,0), BackgroundColor3=Color3.fromRGB(80,160,240), BorderSizePixel=0, Parent=armorBg },{ Create("UICorner",{ CornerRadius=UDim.new(0,2) }) })
		...
		SkelLine(41,6,41,26); SkelLine(41,26,16,34); SkelLine(41,26,66,34); SkelLine(16,34,10,82); SkelLine(10,82,8,116); SkelLine(66,34,72,82); SkelLine(72,82,74,116); SkelLine(41,26,41,100); SkelLine(41,100,26,107); SkelLine(41,100,56,107); SkelLine(26,107,24,152); SkelLine(24,152,22,170); SkelLine(56,107,58,152); SkelLine(58,152,60,170)
		...
		local function updateColor(col)
			espColor=col; accentDot.BackgroundColor3=col; boxStroke.Color=col; fillFrame.BackgroundColor3=col; nameTag.TextColor3=col; chamHL.FillColor=col; chamHL.OutlineColor=col; snapLine.BackgroundColor3=col
		...
		local rotConn=RunService.Heartbeat:Connect(function(dt)
			...
			if cloneHRP and cloneHRP.Parent then local newCF=CFrame.Angles(0,math.rad(rotAngle),0); pcall(function() cloneHRP.CFrame=newCF; for p,rel in pairs(partRelCF) do if p and p.Parent then p.CFrame=newCF*rel end end end) end
			...
			if tickCount>=5 and bindMap then
				...
				local en=tog("enabled"); local col=opt("color"); local filled=tog("filled"); local fillT=opt("fillTrans"); local skel=tog("skeleton"); local headESP=tog("headESP"); local chamsEn=tog("chams"); local chamsC=opt("chamsColor"); local chamsT=opt("chamsTrans"); local armorEn=tog("armor"); local snapEn=tog("snapline"); local weapEn=tog("weapon")
				if col then updateColor(col) end
				local show=en~=false and en~=nil; boxFrame.Visible=show; hbBg.Visible=show; nameTag.Visible=show; distTag.Visible=show
				if filled~=nil then fillFrame.Visible=filled end; if fillT~=nil then fillFrame.BackgroundTransparency=fillT/100 end
				if skel~=nil then skelFrame.Visible=skel end; if headESP~=nil then headDot.Visible=headESP end; if armorEn~=nil then armorBg.Visible=armorEn end; if snapEn~=nil then snapLine.Visible=snapEn end; if weapEn~=nil then weaponTag.Visible=weapEn end
				if chamsEn~=nil then chamHL.Enabled=chamsEn; if chamsC then chamHL.FillColor=chamsC; chamHL.OutlineColor=chamsC end; if chamsT then chamHL.FillTransparency=chamsT/100 end; if chamsEn and charClone and charClone.Parent and not chamHL.Adornee then chamHL.Adornee=charClone end end
			end
		end)
